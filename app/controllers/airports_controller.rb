class AirportsController < ApplicationController
  ...
  # GET /airports.json
  def index
     @airports = Airport.search(params[:search]).order(sort_column + " " + sort_direction).paginate(:per_page => 25, :page => params[:page])
  end
  ...
  private
    ...
    def sort_column
      Airport.column_names.include?(params[:sort]) ? params[:sort] : "name"
    end
